...
# ./main.c:17:     double epsilon = 0;
	pxor	xmm0, xmm0
	movsd	QWORD PTR -64[rbp], xmm0	# epsilon init
# ./main.c:18:     if (argc == 2) {
...
# ./main.c:19:         sscanf(argv[1], "%lf", &epsilon);
	...
	add	rax, 8
	lea	rdx, -64[rbp]   # epsilon => sscanf(argv[1], "%lf", &epsilon);
	lea	rsi, .LC1[rip]    # "%lf" => sscanf(argv[1], "%lf", &epsilon);
	mov	rdi, QWORD PTR [rax]    # argv[1] => sscanf(argv[1], "%lf", &epsilon);
	mov	eax, 0
...
# ./main.c:21:         printf("Epsilon: %*.*lf", 1, 8, epsilon);
	...
	mov	eax, 1
	call	printf@PLT		# printf("Epsilon: %*.*lf", 1, 8, epsilon);
...
# ./main.c:29:         printf("\nTime: %d ms", (int) (difftime(t_end, t_start)) / 1000);
	mov rsi, QWORD PTR -48[rbp]		# t_start => difftime(t_end, t_start)
	mov rdi, QWORD PTR -56[rbp]		# t_end => difftime(t_end, t_start)
	call	difftime@PLT
	cvttsd2si	eax, xmm0
	movsx	rdx, eax
...
# ./main.c:40:         fscanf(input_stream, "%lf", &epsilon);
	lea	rdx, -64[rbp]		# epsilon => fscanf(input_stream, "%lf", &epsilon);
	lea	rsi, .LC1[rip]		# "%lf" => fscanf(input_stream, "%lf", &epsilon);
	mov rdi, QWORD PTR -32[rbp]		# input_stream => fscanf(input_stream, "%lf", &epsilon);
	mov	eax, 0
